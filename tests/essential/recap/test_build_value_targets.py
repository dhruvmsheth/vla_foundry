...
def test_build_value_targets_success_and_failure_returns(tmp_path: Path) -> None:
    ...
    assert summary["normalization_mode"] == "recap"
    assert summary["normalization_horizon"] == 1
    assert summary["normalization_denominator"] == 2.0
    ...
    assert success_first["normalized_value"] == -0.5
    assert success_first["value_bin"] == 5
...
def test_build_value_targets_full_range_returns(tmp_path: Path) -> None:
    labeled_dir = tmp_path / "labeled"
    trajectory_dir = tmp_path / "trajectories"
    labeled_dir.mkdir()
    (labeled_dir / "label_summary.json").write_text(json.dumps({"trajectory_dir": str(trajectory_dir)}))
    write_jsonl(
        labeled_dir / "labeled_steps.jsonl",
        [
            {"episode_id": "success", "step_index": 0, "terminal_is_success": True},
            {"episode_id": "success", "step_index": 1, "terminal_is_success": True},
            {"episode_id": "failure", "step_index": 0, "terminal_is_success": False},
        ],
    )
    ...
    summary = build_value_targets(
        ...
        num_bins=11,
        val_fraction=0.0,
        normalization_mode="full_range",
    )
    examples = [json.loads(line) for line in (tmp_path / "targets" / "value_examples.jsonl").read_text().splitlines()]

    assert summary["normalization_mode"] == "full_range"
    assert summary["normalization_denominator"] == 11.0
    by_episode_step = {(example["episode_id"], example["step_index"]): example for example in examples}
    assert by_episode_step[("success", 0)]["normalized_value"] == -1.0 / 11.0
    assert by_episode_step[("success", 0)]["value_bin"] == 9
    assert by_episode_step[("failure", 0)]["normalized_value"] == -10.0 / 11.0
    assert by_episode_step[("failure", 0)]["value_bin"] == 1
